...
from keras.layers import Dense, Input, Reshape, Lambda, Dropout, concatenate
...
class ICM:
    ...
    def learn(self, prev_states, states, actions, rewards):
        ...
        rewards = np.array(rewards, dtype=float).reshape((-1, 1))
        # dict = {"state_t" : s_t, "state_t1" : s_t1, "action" : actions, "reward_intrinsic" : rewards}
        # print(actions[1])
        # print(rewards)
        # print((np.array(rewards).reshape((-1, 1))))
        assert len(s_t) == self.batch_size and len(s_t1) == self.batch_size and \
               len(actions) == self.batch_size and len(np.array(rewards)) == self.batch_size
        # x = ([np.concatenate([s_t[i], s_t1[i], actions[i], rewards[i]]) for i in range(self.batch_size)])
        x = [pad_sequences([s_t[i], s_t1[i], actions[i], rewards[i]], dtype='float32') for i in range(self.batch_size)]
        # x = [s_t, s_t1, np.array(actions), np.array(rewards).reshape((-1, 1))]
        # print(x)
        # print(len(x))
        # print(len(x[0]))
        ...
        icm_loss = self.model.train_on_batch(x, np.zeros((self.batch_size, 4)))
